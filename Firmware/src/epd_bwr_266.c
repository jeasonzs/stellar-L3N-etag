...
BWR_266_Len, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
#define EPD_BWR_266_test_pattern 0xA5
_attribute_ram_code_ uint8_t EPD_BWR_266_detect(void)
{
    ...
    EPD_WriteCmd(0x32);
    int i;
    for (i = 0; i < 153; i++)  // FIXME  DETECT MODEL 266
    {
        EPD_WriteData(EPD_BWR_266_test_pattern);
    }
    EPD_WriteCmd(0x33);
    for (i = 0; i < 153; i++)
    {
        if(EPD_SPI_read() != EPD_BWR_266_test_pattern)
            return 0;
    }
    return 1;
}
...
_attribute_ram_code_ uint8_t EPD_BWR_266_Display(unsigned char *image, int size, uint8_t full_or_partial) {
    ...
    // Set RAM Y- Address Start/End
    EPD_WriteCmd(0x45);
    EPD_WriteData(0x27);   //0x0127-->(295+1)=266
	EPD_WriteData(0x01);
	EPD_WriteData(0x00);
	EPD_WriteData(0x00);
